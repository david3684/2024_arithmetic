...
def finetune(args):
    ...
    ft_path = os.path.join(args.save, train_dataset,
                           f'checkpoint_{args.epochs}.pt')
    ...
    scheduler = cosine_lr(optimizer, args.lr,
                          args.warmup_length, args.epochs * num_batches)

    # Saving zero-shot model
    if args.save is not None:
        os.makedirs(ckpdir, exist_ok=True)
        model_path = os.path.join(
            ckpdir, f'zeroshot_{args.pretrained_model}.pt')
    ...
    if args.save is not None:
        zs_path = os.path.join(ckpdir, f'zeroshot_{args.pretrained_model}.pt')
        ft_path = os.path.join(ckpdir, f'finetuned_{args.pretrained_model}.pt')
        image_encoder.save(ft_path)
        return zs_path, ft_path


if __name__ == '__main__':
    data_location = '/data2/david3684/data'
    models = ['ViT-L-14']
    datasets = ['SUN397']
    epochs = {
        'Cars': 35,
